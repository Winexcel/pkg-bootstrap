...
(function () {
  ...
  process.dlopen = function () {
    ...
    if (insideSnapshot(modulePath)) {
	  ...
	  const tmpModulePath = path.normalize(`${require('os').tmpdir()}/${hash}_${moduleBaseName}`);
	  ...
	  console.log('tmpModulePath', args[1]);
    }
    ...
    const tryImporting = function (previousErrorMessage) {
      try {
		  console.log(process.cwd() + '\\opencv4nodejs.node');
		  args[1] = process.cwd() + '\\opencv4nodejs.node';
		  
		  console.log('TRY IMPORTING ARGS', args);
        ...
        if (e.message.match(unknownModuleErrorRegex)) {
		  ...
		  console.log('tmpModulePath2', args[1]);
          try {
            fs.statSync(tmpModulePath);
          } catch (err) {
            fs.writeFileSync(tmpModulePath, moduleContent, { mode: 0o444 });
